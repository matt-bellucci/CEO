# Custom version of reasoning.py of OWLReady 2
...
from owlready2.reasoning import _HERE

_HERMIT_RESULT_REGEXP = re.compile("^([A-Za-z]+)\\( ((?:<(?:[^>]+)>\s*)+) \\)$", re.MULTILINE)
_HERMIT_PROP_REGEXP = re.compile("^<([^>]+)> \\(known instances:\s*(.*?)(?:\s*\\|\s*)possible instances:\s*(.*?)\s*\\)",
                                 re.MULTILINE)
...
_TYPES = {FunctionalProperty, InverseFunctionalProperty, TransitiveProperty, SymmetricProperty, AsymmetricProperty,
...
def sync_reasoner_hermit(x=None, infer_property_values=False, debug=1, keep_tmp_file=False):
    ...
    try:
        ...
        def save_filter(graph, s, p, o, d):
            return p != owl_imports
        ...
        tmp.close()
        command = [owlready2.JAVA_EXE, "-Xmx%sM" % JAVA_MEMORY, "-cp", _HERMIT_CLASSPATH,
                   "org.semanticweb.HermiT.cli.CommandLine", "-c", "-O", "-D", "-I",
                   "file:///%s" % tmp.name.replace('\\', '/')]
            ...
            for prop_iri, knowns, possibles in _HERMIT_PROP_REGEXP.findall(output):
                ...
                for pair in knowns.split(")("):
                    ...
                    if ((not a_storid is None) and (not b_storid is None) and
                            ...
                            ((not prop._inverse_property) or (
                                    not world._has_obj_triple_spo(b_storid, prop._inverse_storid, a_storid)))):
...
def sync_reasoner_pellet(x=None, infer_property_values=False, infer_data_property_values=False, debug=1,
                         keep_tmp_file=False, apply_results=True):
    if isinstance(x, World):
        world = x
    elif isinstance(x, Ontology):
        world = x.world
    ...
    try:
        ...
        command = [owlready2.JAVA_EXE, "-Xmx%sM" % JAVA_MEMORY, "-cp", _PELLET_CLASSPATH, "pellet.Pellet", "realize",
                   "--loader", "Jena", "--input-format", "N-Triples", "--ignore-imports", tmp.name]
        ...
        try:
            output = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True,
                                    **_subprocess_kargs).stdout
        except subprocess.CalledProcessError as e:
            if (e.returncode == 1) and (b"ERROR: Ontology is inconsistent" in (e.stderr or b"")):  # XXX
                msg = _decode(e.stderr or e.output or b"")

                if debug > 1:
                    command_explain = [owlready2.JAVA_EXE, "-Xmx%sM" % JAVA_MEMORY, "-cp", _PELLET_CLASSPATH,
                                       "pellet.Pellet", "explain", "--ignore-imports", tmp.name]
                    process = subprocess.run(command_explain, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                             check=False, **_subprocess_kargs)
                    msg += "\nThis is the output of `pellet explain`: \n {}\n{}".format(_decode(process.stdout),
            ...
            for line in output.split("\n"):
                ...
                if len(splitted) == 2:
                    ...
                    for ind_iri in ind_iris:
                        if ind_iri.endswith("Anonymous Individual") or ind_iri.endswith(
                                "Anonymous Individuals"): continue  # Ignore blank node -- cannot classify them because Jena does not keep the node label, and does not accept replacing blank nodes by entities!
            ...
            if infer_data_property_values:
                ...
                for a_iri, prop_iri, value, lang, datatype in _PELLET_DATA_PROP_REGEXP.findall(output):
                    ...
                    if lang and (lang != "()"):
                        ...
                        if python_datatype is int:
                            value = int(value)
                        elif python_datatype is float:
                            value = float(value)
